...
static void test_parse()
{
	...
	bool b;
	assert(!sipc_bool(&p, &b) && b == true);
	assert(!sipc_bool(&p, &b) && b == false);

	unsigned u;
	assert(!sipc_uint(&p, &u) && u == 0);
	assert(!sipc_uint(&p, &u) && u == 0xff);
	assert(!sipc_uint(&p, &u) && u == 0x100);
	assert(!sipc_uint(&p, &u) && u == 0x180);
	assert(!sipc_uint(&p, &u) && u == 0x1000);
	assert(!sipc_uint(&p, &u) && u == 0x80000000);

	int i;
	assert(!sipc_int(&p, &i) && i == -0xff);
	assert(!sipc_int(&p, &i) && i == -0x70000000);

	double d;
	assert(!sipc_double(&p, &d) && d == 0x1abcdp-14);
	assert(!sipc_double(&p, &d) && isnan(d));
	assert(!sipc_double(&p, &d) && isinf(d) && d > 0);
	assert(!sipc_double(&p, &d) && isinf(d) && d < 0);
	assert(!sipc_double(&p, &d) && d == 0.0);
	assert(!sipc_double(&p, &d) && d == 128.0);
	assert(!sipc_double(&p, &d) && d == 256.0);

	const char *string;
	assert(!sipc_string(&p, &i, &string) && i == 3 &&
	       !strncmp(string, "abc", 3));

	const unsigned char *bytes;
	assert(!sipc_bytes(&p, &i, &bytes) && i == 3 &&
	       !strncmp((char *)bytes, "123", 3));

	sipc_any_t any;
	assert(!sipc_any(&p, &any) && any.type == SIPC_DOUBLE &&
	       any.d == 0x1abcdp-14);

	assert(!sipc_any(&p, &any) && any.type == SIPC_END);
